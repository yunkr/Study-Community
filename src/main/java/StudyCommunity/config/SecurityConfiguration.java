package StudyCommunity.config;

import StudyCommunity.auth.filter.JwtAuthenticationFilter;
import StudyCommunity.auth.filter.JwtVerificationFilter;
import StudyCommunity.auth.handler.*;
import StudyCommunity.auth.jwt.JwtTokenizer;
...
//                                .requestMatchers(HttpMethod.POST, "/*/studies").hasRole("USER")
                ...
                .oauth2Login(oauth2 ->
                        oauth2.successHandler(new OAuth2MemberSuccessHandler(jwtTokenizer, authorityUtils, memberRepository))
                );
